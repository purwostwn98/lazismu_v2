...
jQuery(function($) {
  var tableId = '#datatable2';
  var tableWapperId = '#datatable_wrapper';
  var tableHeader = '#table-header2';


  $.extend( true, $.fn.dataTable.defaults, {
    dom:
      "<'row'<'col-12 col-sm-6'l><'col-12 col-sm-6 text-right table-tools-col2'f>>" +
      "<'row'<'col-12'tr>>" +
  ...
  var $_table = $(tableId).DataTable({
    ...
    language: {
      ...
      searchPlaceholder: " Search ajuan ..."
    }
  });

  //specify position of table buttons
  $('.table-tools-col2' )//specified above in $.fn.dataTable.defaults
  .append( $_table.buttons().container() )
  //move searchbox into table header
  .find('.dataTables_filter').appendTo('.page-tools2').find('input').addClass('pl-4 radius-round')
